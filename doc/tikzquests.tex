...
\setnewcodekey{tikzquests}{letter={},
  ...
  emph3={tikz},
  emph2={xtrakeys,xtraidx,no, defs,old, settings,no, alias,in, review, undef, color, local , keys, prefix}
  }
...
\describe{draft}{(default: false). When using a question (see \ref{question use}), the question's associated remarks and annotations will be printed as well, if any.}
\describe{in review}{This is an alias to \tsobj[option]{draft}}
...
\describe{keys prefix}{This will add a prefix to all variables. It defaults to `nothing' (no prefix).}
...
By default the set of \tsobj[pkg]{keys} and macros is defined as follow: \tsobj[oarg]{prefix}\tsobj[marg]{ID}\tsobj[marg]{idx}. Default \tsobj[marg]{prefix} being nothing. The predefined range of \tsobj[marg]{IDs} being \tsobj[key]{R,L,C,Z,Y,G,H,X,K,T,Q,D,EQ,V,I}. The predefined range of \tsobj[marg]{idx} spans from a, b, c up to z, aa, ab, ac up to az and, finally, ba, bb, bc up to bz. 
\begin{tsremark}
...
All those macros/keys are initialized  as follow (math mode) \tsobj{\ensuremath{<prefix><ID>_{<idx>}}}, resulting in $prefixID_{idx}$. So, for example, the key \tsobj[key]{Rab}, which can be accessed with the macro \tsobj{\Rab},  will be predefined as $R_{ab}$, \tsobj[key]{Lca} (\tsobj{\Lca}) will be predefined as $L_{ca}$, and so on.
\begin{tsremark}[\color{red}Note:]
  If \tsobj[option]{keys prefix} (see \ref{package options}) is set, all \emph{pre-defined} keys will have that prefix, but only then. For instance, if one set's \tsobj[option]{keys prefix = kp} (when loading this package), the same example above would be, key \tsobj[key]{kpRab}, macro \tsobj{\kpRab}, default value $kpR_{ab}$, and so on.
\end{tsremark}
...
\end{codedescribe}
\begin{tsremark}
  If \tsobj[option]{keys prefix} (see \ref{package options}) is set, all \emph{pre-defined} keys will have that prefix, it's part of their name.
\end{tsremark}
...
This is the default case, in which both \tsobj{\QuestVal} and (for default keys) associated macro name can be used to retrieve a key/parameter value, and no prefix will be added to any key.
...
\item Ra, Rb ... Rz, Raa, Rab ... Raz .... \emph{Rfa, Rfb ... Rfz, Rha, Rhb ... Rhz}
\item .... all other default sets of keys, plus
\item EXa, EXb ... EXz, EXaa, EXab ... EXaz .... \emph{EXfa, EXfb ... EXfz, EXha, EXhb ... EXhz}
\item Na, Nb ... Nz, Nab, Naa ... Naz .... \emph{Nfa, Nfb ... Nfz, Nha, Nhb ... Nhz}
\end{itemize}

Besides that, the undefined color will be blue and no additional macro will be defined, meaning that \tsobj{\QuestVal} is the only option to retrieve the value of a key/parameter.\\[5mm]


\begin{codestore}[pack.options D]
\usepackage[xtrakeys={EX,N},keys prefix = {pr}]{tikzquests}
\end{codestore}

\tscode*[tikzquests,codeprefix={Package Options}]{pack.options D}

In this case, one will get the following set of keys

\begin{itemize}[midsep]
\item prRa, prRb ... prRz, prRaa, prRab ... prRaz .... 
\item .... all other default sets of keys, plus
\item prEXa, prEXb ... prEXz, prEXaa, prEXab ... prEXaz ....
\item prNa, prNb ... prNz, prNaa, prNab ... prNaz ....
\end{itemize}

Note the prefix `pr' in all keys.\\[5mm]



\begin{codestore}[pack.options E]
\usepackage[keys={R,L,C},idx={{},a},keys prefix = {pr},local defs]{tikzquests}
\end{codestore}

\tscode*[tikzquests,codeprefix={Package Options}]{pack.options E}

In this case, one will only get the following set of keys

\begin{itemize}[midsep]
\item prRa, prRb ... prRz, prRaa, prRab ... prRaz 
\item prLa, prLb ... prLz, prLaa, prLab ... prLaz 
\item prCa, prCb ... prCz, prCaa, prCab ... prCaz 
\end{itemize}

Note that, these keys will be defined only locally, when using \tsobj[code,or]{\Question,\tikzQuestion,\ftikzQuestion}.\\[5mm]
...
In this case, no alias will be defined (the command \tsobj{\defQuestionAlias} will be ignored), and when using \tsobj{\tikzQuestion} (and similar) the question's remarks (defined by \tsobj{\defQuestion}) and annotations (from \tsobj{\tikzQuestion}) will be printed. The \tsobj[option]{no alias} is specially useful when using the commands \tsobj[code,or]{\QuestionsFmtList,\QuestionsList}.
...
%\tikzQuestion(0.35)[Repo 2]{Elect. 1a}

%\ExplSyntaxOn
%\cs_show:N \exp_last_unbraced:Nno
%\ExplSyntaxOff
